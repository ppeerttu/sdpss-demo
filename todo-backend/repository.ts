import { PersistedUser, PersistedTodo } from "./models.ts";
...
type TodosRow = Omit<PersistedTodo, "createdAt" | "updatedAt"> & { created_at: Date; updated_at: Date; };
...
const mapRowToTodo = ({ created_at, updated_at, ...rest }: TodosRow): PersistedTodo => ({
    ...rest,
    createdAt: created_at,
...
export const listTodos = async (userId?: string): Promise<PersistedTodo[]> => {
    const res = await (
        userId
            ? client.queryObject<TodosRow>`SELECT * FROM todos WHERE user_id = ${userId}`
...
export const completeTodo = async (todoId: string, completed = true): Promise<PersistedTodo | null> => {
...
export const findUser = async (username: string): Promise<PersistedUser | null> => {
    const res = await client.queryObject<UsersRow>`SELECT * FROM users WHERE username = ${username}`;
